...
app.post('/signup', async (req, res) => {
  ...
  try {
    // const hashedPW = await axios.get('http://auth/hashed-password/' + password); // docker internal communication
...
app.post('/login', async (req, res) => {
  ...
  // );
  const response = await axios.get(
    `${process.env.AUTH_SERVICE_SERVICE_HOST}/token/` + hashedPassword + '/' + password
  );
